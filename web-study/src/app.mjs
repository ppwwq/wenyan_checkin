...
import {validTraining,trainingOptions,dailyProgress,dailyQuestions,retryQuestions} from './domain/training.mjs';
...
async function sync(){
 ...
 try{const result=await current.sync();if(current!==syncer||!result)return;serverReports=result.reports||[];corrections=result.corrections||[];await active.set('corrections',corrections);await refresh();if(!['practice','library','training'].includes(page))render();}
...
async function openAccount(){
 ...
 syncer=new SyncClient(repo,auth.token,setStatus);render();await sync();if(!validTraining(state.settings.training)){page='training';render();}
}
function render(){
 if(!auth){authView();return;}
 document.body.classList.toggle('in-session',page==='practice');const activePage=['weakness','saved','quick'].includes(page)?'home':['practice','compare'].includes(page)?'library':page;const navHtml=(mobile=false)=>nav.filter(([id])=>!mobile||!['weakness','saved','quick'].includes(id)).map(([id,label],i)=>'<button data-action="nav" data-page="'+id+'" class="'+(activePage===id?'active':'')+'"'+(activePage===id?' aria-current="page"':'')+'>'+(mobile?'<span class="nav-icon">'+navigationIcon(id)+'</span>':'')+label+'</button>').join('');
 root.innerHTML='<div class="shell"><aside class="sidebar"><div class="brand"><img class="brand-icon" src="/assets/icon.svg" alt="" width="44" height="44"><div><div class="brand-name">DSE文言练习</div><small class="muted">十六篇 · 練習與複習</small></div></div><nav class="nav" aria-label="主要導航">'+navHtml()+'</nav><div class="sidebar-bottom">'+button('training-settings','學習設定 ↗','','quiet')+'</div></aside><div class="workspace"><header class="topbar"><div class="mobile-brand"><img class="brand-icon" src="/assets/icon.svg" alt="" width="44" height="44">DSE文言练习</div><div class="breadcrumb"><b>'+esc(nav.find(n=>n[0]===page)?.[1]||'練習')+'</b></div><div class="top-actions"><span id="sync-status" class="status-line" role="status">'+esc(status)+'</span><button data-action="settings" aria-label="調整全站字號">字號 Aa</button></div></header><main class="page" id="main"></main></div></div><nav class="mobile-nav" aria-label="行動導航">'+navHtml(true)+'</nav>';
 const views={home:()=>homeView(bank,state),training:()=>trainingView(bank,state,options),library:()=>libraryView(bank,state,options),practice:()=>session?practiceView(bank,session,state):empty('沒有進行中的練習'),weakness:()=>weaknessView(bank,state,options),history:()=>historyView(bank,state),saved:()=>savedView(bank,state,options),quick:()=>quickView(bank,state,options,quick),compare:()=>compareView(bank,options,compareTarget,compareExpanded),reports:()=>reportsView(mergedReports()),account:accountView,admin:adminView};
...
async function resumeSession(id){
 ...
 session=structuredClone(state.sessions[id]);if(!session)return;
 const draft=await repo.get('draft-session/'+id);if(draft&&(draft.revision||0)>(session.revision||0))session=draft;
 for(const a of state.attempts.filter(e=>e.payload.sessionId===id))session.answers[a.payload.questionId]=a.id;
 if(session.kind==='retry'){
  ...
  session.index=session.questions.findIndex(q=>!session.answers[q.id]);
...
async function beginSession(list,opts,kind){
 ...
 session={id:crypto.randomUUID(),kind,goalDay:hkDay(),goalSnapshot:dailyProgress(state).goal,questions:structuredClone(list),index:0,answers:{},drafts:{},mode:state.settings.typing?'mixed':'choice',essayIds:opts.essayIds,completed:false,paused:false,startedAt:new Date().toISOString()};
...
async function startDaily(kind='daily'){
 await saveChain;await sync();if(!validTraining(state.settings.training)){await go('training');return;}
...
document.addEventListener('click',async e=>{
 ...
 if(a==='daily-count'){document.querySelector('#daily-count').value=el.dataset.count;return;}
 ...
 if(a==='nav'&&el.dataset.page==='library'&&el.closest('nav.nav,nav.mobile-nav')){await go('home');return;}
 ...
 else if(a==='wrong-practice'){options.scope='wrong';options.questionIds=undefined;await saveOptions();await go('library');}
...
document.addEventListener('submit',async e=>{
 ...
 if(form.id==='training-form'){
  ...
  await append('settings',{training});await go('home');void sync();toast('學習設定已保存；之後可直接開始每日訓練。');
 }
...
document.addEventListener('change',async e=>{
 try{
 if(e.target.dataset.trainingEssay){const id=e.target.dataset.trainingEssay;const types=[...document.querySelectorAll('[data-types-for]')].find(el=>el.dataset.typesFor===id);if(types)types.hidden=!e.target.checked;return;}
