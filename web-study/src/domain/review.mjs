...
export function project(input, today = hkDay(), corrections = []) {
  ...
  const dailyGoals = {}, failures = new Map();
  for(const e of attempts){const p=e.payload,day=hkDay(p.submittedAt),correct=correctionMap.has(e.id)?correctionMap.get(e.id).correct:p.mode==='typing'?(assessments.get(e.id)??null):p.correct;if(p.trainingPolicy===2&&correct===false)failures.set(p.memoryId+'/'+day,p);}
  ...
  for (const f of firsts) {
    const old = memories[f.memoryId], base = {...old,...f,hasFailure:false,errors:old?.errors||0,reviews:old?.reviews||0};
    if (f.correct === null) { memories[f.memoryId] = {...(old||{memoryId:f.memoryId,correct:null,errors:0,reviews:0}),pending:true,pendingAttempt:f}; }
    else if (!f.correct) {
    ...
    if(failures.has(f.memoryId+'/'+f.day)){const m=memories[f.memoryId];m.hasFailure=true;if(f.correct!==false)m.errors=base.errors+1;m.stage=0;m.interval=1;m.due=addDays(f.day,1);m.againAt=null;}
  }
  ...
  return {events,attempts,assessments,firsts,memories,favorites,sessions,settings,dailyGoals,reports,browsed,completedToday,corrections};
}
