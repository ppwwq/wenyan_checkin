...
export function answerPanel(q,session,state){
 ...
 let html='<div class="question-top"><span class="question-tag">'+esc(abilities[q.ability])+' · '+(mode==='typing'?'文字自查':'單項選擇')+'</span><button class="chip" data-action="favorite" data-id="'+esc(q.id)+'">'+(state.favorites[q.id]?.saved?'★ 已收藏':'☆ 收藏')+'</button></div><h2>'+esc(q.stem)+'</h2>';
 ...
 html+='<section class="feedback" tabindex="-1"><div class="verdict '+(p.correct===false?'miss':'')+'">'+(mode==='typing'?'對照答案，完成自查':p.correct?'這次答對了':'把這個語境再讀一次')+'</div><details class="attempt-note"><summary>作答記錄</summary><p>'+ (first?.id===attempt.id?'今天首次作答':'今天重練 · 不改變複習間隔')+'</p></details><div class="explanation"><h3>參考答案</h3><p>'+esc(q.choices.find(c=>c.id===q.answerId)?.text)+'</p><p class="explanation-detail">'+esc(q.explanation)+'</p><details><summary>逐項辨析</summary>'+q.choices.map(c=>'<p class="choices-explained">'+esc(c.id.toUpperCase())+' · '+esc(c.explanation)+'</p>').join('')+'</details></div>';
