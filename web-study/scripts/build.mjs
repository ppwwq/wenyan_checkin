...
for(const path of ['src/app.mjs','src/domain/review.mjs','src/domain/queue.mjs','src/features/shared.mjs','src/features/preferences.mjs','src/features/practice.mjs','src/features/library.mjs','src/features/records.mjs','src/sync/client.mjs','src/storage/repository.mjs','sw.js'])execFileSync(process.execPath,['--check',resolve(root,path)]);
await mkdir(resolve(root,'dist'),{recursive:true});
for(const file of files)await cp(resolve(root,file),resolve(root,'dist',file),{recursive:true});
await writeFile(resolve(root,'dist/build-info.json'),JSON.stringify({uiVersion:'2026.09.20.3',version:bank.version,questions:bank.questions.length,builtAt:new Date().toISOString()},null,2));
