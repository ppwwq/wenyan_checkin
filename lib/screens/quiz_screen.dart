...
class _QuizScreenState extends State<QuizScreen> {
  ...
  String _segmentOriginal = '';
  String _segmentTranslation = '';
  ...
  Future<void> _loadQuestionContext(Map<String, dynamic> question) async {
    ...
    // Resolve the translation segment that contains the annotated word, so the
    // answer feedback can also show the 原文翻譯.
    _segmentOriginal = '';
    _segmentTranslation = '';
    if (_annotationWord.isNotEmpty) {
      final translations = await contentService.getTranslations(essayId);
      Map<String, dynamic>? matchedTranslation;
      for (final t in translations) {
        final seg = t['original_segment'] as String? ?? '';
        if (seg.contains(_annotationWord)) {
          matchedTranslation = t;
          break;
        }
      }
      if (matchedTranslation != null) {
        _segmentOriginal = matchedTranslation['original_segment'] as String? ?? '';
        _segmentTranslation = matchedTranslation['translation'] as String? ?? '';
      }
    }
  }
    ...
    return SingleChildScrollView(
      child: Column(
        ...
        if (_lastAnsweredIndex == questionIndex && explanation.isNotEmpty) ...[
          ...
          // Original text translation (if the word maps to a translated segment)
          if (_segmentTranslation.isNotEmpty) ...[
            const SizedBox(height: 10),
            Container(
              width: double.infinity,
              padding: const EdgeInsets.all(14),
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('原文翻譯', style: TextStyle(fontSize: 12, color: AppTheme.secondary)),
                  const SizedBox(height: 6),
                  if (_segmentOriginal.isNotEmpty) ...[
                    Text(
                      _segmentOriginal,
                  ...
                  Text(
                    _segmentTranslation,
                    style: const TextStyle(fontSize: 15, color: AppTheme.secondary, height: 1.7),
                  ),
                ],
              ),
            ),
