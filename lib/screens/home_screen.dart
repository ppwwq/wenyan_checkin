...
class _HomeScreenState extends State<HomeScreen> {
  ...
  int _dailyTarget = 5;
  static const _targetOptions = [5, 10, 15, 20, 30, 50];
  ...
  Future<void> _editDailyTarget() async {
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: DropdownButtonFormField<int>(
            initialValue: _targetOptions.contains(_dailyTarget) ? _dailyTarget : _targetOptions.last,
            decoration: const InputDecoration(
              border: OutlineInputBorder(),
              labelText: '每日練習題數',
            ),
            items: _targetOptions
                .map((n) => DropdownMenuItem<int>(value: n, child: Text('$n 題')))
                .toList(),
            onChanged: (v) {
    ...
    // Real-time update: persist + refresh the header "每日 X 題" as soon as a
    // value is picked from the dropdown.
