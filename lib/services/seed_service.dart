...
class SeedService {
  ...
  static Future<int> _insertEssayWithRelations(DatabaseExecutor db, Map<String, dynamic> data) async {
    ...
    // Insert annotations, remembering their auto-increment ids by index
    final annotations = data['annotations'] as List? ?? [];
    final annotationIds = <int>[];
    for (int j = 0; j < annotations.length; j++) {
      final ann = annotations[j];
      final annId = await db.insert('annotations', {
        'essay_id': essayId,
        'word': ann['word'],
        'meaning': ann['meaning'],
        'position': j,
    ...
    // Insert auto-generated questions, binding each to its source annotation.
    // The generator produces one question per annotation in the same order, so
    // question[j] corresponds to annotation[j]. Fall back to meaning matching
    // if the counts differ.
    final questions = data['questions'] as List? ?? [];
    final indexAligned = questions.length == annotationIds.length;
    for (int j = 0; j < questions.length; j++) {
      final q = questions[j];
      int? annotationId;
      if (indexAligned && j < annotationIds.length) {
        annotationId = annotationIds[j];
      }
      if (annotationId == null) {
        // Meaning-based fallback (first unambiguous match within this essay)
        final matches = annotations
            .where((a) => a is Map && a['meaning'] == q['correct_answer'])
            .toList();
        if (matches.length == 1) {
          annotationId = annotationIds[annotations.indexOf(matches.first)];
        }
      }
