...
class StreakService {
  ...
  Future<Map<String, dynamic>> getStats(int userId) async {
    ...
    // Real answer accuracy = correct attempts / total attempts, counted over the
    // word (annotation) records only — each answered question maps to exactly one.
    final answerCount = Sqflite.firstIntValue(await _db.rawQuery(
      "SELECT SUM(review_count) FROM study_records WHERE user_id = ? AND target_type = 'annotation'",
      [userId])) ?? 0;
    final correctCount = Sqflite.firstIntValue(await _db.rawQuery(
      "SELECT SUM(correct_count) FROM study_records WHERE user_id = ? AND target_type = 'annotation'",
      [userId])) ?? 0;
    return {
      'accuracy': answerCount > 0 ? (correctCount / answerCount * 100).round() : 0,
