...
本次 GitHub 同步快照包含介面 `2026.09.24.7`、題庫 `2026.09.23.22`、來源PDF及題庫修訂工具／記錄。[同步範圍與核查](web-study/verification/github-sync-2026-09-24/README.md)。GitHub 推送與網站部署分開；本次沒有部署。

本地介面已修復至 `2026.09.24.7`：錯題入口、實際錯題篩選及計數；77項測試與隔離瀏覽器檢查通過。**本次未部署，正式站仍 `2026.09.24.6`**。[修復與核查](web-study/verification/wrong-entry-2026-09-24/README.md)。

以下表格為上次正式發布狀態：

| 項目 | 狀態 |
|---|---|
| 上次發布介面 | `2026.09.24.6`，已部署並核查 |
| 本地／正式題庫 | `2026.09.23.22`；2,134題，2,133啟用題；本輪內容未改 |
| 主要修改 | 保存錯誤重試、事務式備份恢復、自動補傳、手機狀態與薄弱卡片、簡化設定、完成頁實際篇章 |
| 驗證 | 73項測試、真實IndexedDB恢復檢查、手機／平板瀏覽器流程、36個線上檔案一致 |

[本輪修改與核查](web-study/verification/three-stages-2026-09-24/README.md) · [正式站核查](web-study/verification/release-2026.09.23.22-ui-2026.09.24.6/public-result.json)。真iPad／Safari及學生試用仍待進行。以下帶較早版本的段落為歷史記錄，不代表目前正式版本。
...
歷史內容審查紀錄：題庫 `2026.09.23.6`：2,134題／2,084記憶單元。252題經本輪逐題交叉審查；其餘1,882題沿用舊題及舊記錄，未重新逐題語義審讀。[最新部署核查](web-study/verification/release-2026.09.23.6/public-result.json)與[低耗用去向](tools/question-bank/quality-revision/fast-disposition.md)分別記錄線上版本和後續待辦。下方舊版數字保留作歷史說明。
...
2026.09.22.1 版本共 **2,134題、2,084個記憶單元、11組詞義對照**，當時新增3道「歸」的跨篇易混詞題；此處是歷史記錄，最新正式站版本見本文開頭。

| 範圍 | 題數 | 本版處理 |
| --- | ---: | --- |
| 指定篇章練習 | 1,983 | 原1,921道修訂題及59道保留題不變，新增3道跨篇易混詞題。 |
